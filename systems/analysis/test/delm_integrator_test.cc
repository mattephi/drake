#include "drake/systems/analysis/delm_integrator.h"
...
GTEST_TEST(IntegratorTest, MiscAPI) {
  ...
  DELMIntegrator<double> int_dbl(spring_mass_dbl, h,
                                          context_dbl.get());
  DELMIntegrator<AutoDiffXd> int_ad(spring_mass_ad, h,
                                             context_ad.get());
...
GTEST_TEST(IntegratorTest, ContextAccess) {
  ...
  DELMIntegrator<double> integrator(
...
GTEST_TEST(IntegratorTest, Symbolic) {
  ...
  auto context = spring_mass.CreateDefaultContext();
  // Create the integrator.
  DELMIntegrator<Expression> integrator(
      spring_mass, max_h, context.get());
  integrator.Initialize();
